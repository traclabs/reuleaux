...
octomap::OcTree* SphereDiscretization::generateSphereTree2(const octomap::point3d& origin, float radius, float resolution)
{
   ...
   * @brief Generate a box that goes [-1.5*radius, +1.5*radius] 
   * @brief Around the origin in XYZ
   */
octomap::OcTree* SphereDiscretization::generateBoxTree(const octomap::point3d& origin,
						       float radius,
						       float resolution)
{
  ...
  octomap::Pointcloud p;
  float factor = 1.5;
  for (float x = origin.x() - radius * factor;
       x <= origin.x() + radius * factor;
       x += resolution)
  {
    for (float y = origin.y() - radius * factor;
	 y <= origin.y() + radius * factor;
	 y += resolution)
    {
      for (float z = origin.z() - radius * factor;
	   z <= origin.z() + radius * factor;
	   z += resolution)
      {
...
/**
 * @function get_sphere_num_poses
 */  
int SphereDiscretization::get_sphere_num_pose_divisions()
{
  return (int)(2*divider_*divider_);
}
  
/**
 * @function make_sphere_poses
 * @brief Gets input the origin and the radius, returns 50 poses on the sphere's surface
 */
void SphereDiscretization::make_sphere_poses(const octomap::point3d& origin,
					     double r,
					     std::vector< geometry_msgs::Pose >& pose_Col)
{
  const double DELTA = M_PI /divider_;
  const unsigned MAX_INDEX = (unsigned)(get_sphere_num_pose_divisions());
...
/**
 * @function convertPointToVector
 * @brief Convert octomap point3d to vector of doubles
 */
void SphereDiscretization::convertPointToVector(const octomap::point3d point,
...
/**
 * @function convertPoseToVector
 * @brif Convert pose message to 7-element vector of doubles
 */
void SphereDiscretization::convertPoseToVector(const geometry_msgs::Pose& pose,
					       std::vector< double > &data)
{
  data.resize(7);
