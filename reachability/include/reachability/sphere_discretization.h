...
public:
  ...
  int get_sphere_num_pose_divisions();
  
  //! Generating a sphere octree by insertRay
  octomap::OcTree* generateSphereTree(const octomap::point3d& origin,
				      float radius,
				      float resolution);

  //! Generating a sphere octree by creating poincloud and pushing to octree
  octomap::OcTree* generateSphereTree2(const octomap::point3d& origin,
				       float radius,
				       float resolution);
  ...
  void make_sphere_poses(const octomap::point3d &origin,
			 double r,
			 std::vector< geometry_msgs::Pose >& pose_Col);

  //! Creating random doubles
  double irand(int min,
	       int max);

  //! Creating sphere with random points
  octomap::Pointcloud make_sphere_rand(octomap::point3d origin,
				       double r,
				       int sample);

  //! Creating sphere by Archimedes theorem
  octomap::Pointcloud make_sphere_Archimedes(octomap::point3d origin,
					     double r,
					     int sample);

  //! Creating sphere by Fibonacci grid
  octomap::Pointcloud make_sphere_fibonacci_grid(octomap::point3d origin,
						 double r,
						 int sample);

  //! Creates sphere spiral points and returns pointcloud
  octomap::Pointcloud make_sphere_spiral_points(const octomap::point3d &origin,
						double r,
						int sample);

  //! Creates long lat grid on a sphere
  octomap::Pointcloud make_long_lat_grid(const octomap::point3d &origin,
					 double r,
					 int sample,
					 int lat_num,
					 int lon_num);
